#!/bin/bash
# Build basic-tester on Linux/macOS.
set -e
...
RUNNER_SRC_DIR="$REPO_ROOT/src/tests/basic-tester"

BUILD_TYPE="${1:-Debug}"
ARCH="${2:-}"

if [[ -n "$ARCH" ]]; then
  CMAKE_BUILD_DIR="$(leanclr_cmake_build_dir tests/basic-tester "$BUILD_TYPE" "$ARCH")"
else
  CMAKE_BUILD_DIR="$(leanclr_cmake_build_dir tests/basic-tester "$BUILD_TYPE")"
fi
