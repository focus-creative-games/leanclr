...
call "%REPO_ROOT%\scripts\lib\cmake-dir.bat" "samples\custom-pinvoke-x64" "%CONFIG%" "%ARCH%"
...
cmake --build "%CMAKE_BUILD_DIR%" --config %CONFIG% --target custom-pinvoke --parallel
if errorlevel 1 goto :error

set "EXE=%CMAKE_BUILD_DIR%\bin\%CONFIG%\custom-pinvoke.exe"
if exist "%EXE%" (
  echo Built: "%EXE%"
) else (
