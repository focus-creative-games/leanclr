...
call "%REPO_ROOT%\scripts\lib\cmake-dir.bat" "samples\startup" "%CONFIG%" "%ARCH%"
...
echo [2/2] Build target 'startup'...
cmake --build "%CMAKE_BUILD_DIR%" --config %CONFIG% --target startup --parallel
if errorlevel 1 goto :error

set "EXE=%CMAKE_BUILD_DIR%\bin\%CONFIG%\startup.exe"
if exist "%EXE%" (
  echo Built: "%EXE%"
) else (
