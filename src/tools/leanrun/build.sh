...
BUILD_TYPE=${1:-Debug}
CMAKE_BUILD_DIR="$(leanclr_cmake_build_dir tools/leanrun "$BUILD_TYPE")"

echo "Build dir: $CMAKE_BUILD_DIR"
mkdir -p "$CMAKE_BUILD_DIR"

cmake -S "$SCRIPT_DIR" -B "$CMAKE_BUILD_DIR" -DCMAKE_BUILD_TYPE="$BUILD_TYPE"
cmake --build "$CMAKE_BUILD_DIR" --target leanrun -- -j$(nproc 2>/dev/null || sysctl -n hw.ncpu)

EXE="$CMAKE_BUILD_DIR/bin/leanrun"
if [ -f "$EXE" ]; then
  echo "Built: $EXE"
else
