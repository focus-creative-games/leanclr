...
template <size_t SLOT_GRANULARITY, size_t SEGMENT_SIZE_BYTE_COUNT>
class SegmentedAddressBitmap
{
  private:
    ...
    static constexpr size_t kSegmentCacheSize = 1024;
    static_assert((kSegmentCacheSize & (kSegmentCacheSize - 1)) == 0, "kSegmentCacheSize must be a power of two");
    ...
    mutable SegmentCacheEntry _segment_cache[kSegmentCacheSize]{};
    ...
    {
        const size_t cache_slot = segment_index & (kSegmentCacheSize - 1);
